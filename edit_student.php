...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$error) {
        ...
        $stmt->bind_param(
            "sssssssssdssi",
            $name, $email, $dob, $gender, $phone, $address, $course,
            $enrollment_id, $year, $gpa, $advisor, $notes, $id
        );
...
/* Sidebar */
.sidebar {width:260px;background:#1f2937;color:white;display:flex;flex-direction:column;transition:0.3s;}
...
<div class="main">
    ...
    <form method="post">
        ...
        <input type="text" name="enrollment_id" value="<?php echo htmlspecialchars($student['enrollment_id']); ?>">

        <label>Year / Semester</label>
        <input type="text" name="year" value="<?php echo htmlspecialchars($student['year']); ?>">

        <label>GPA / Grade</label>
        <input type="number" step="0.01" name="gpa" value="<?php echo htmlspecialchars($student['gpa']); ?>">

        <label>Advisor / Teacher</label>
        <input type="text" name="advisor" value="<?php echo htmlspecialchars($student['advisor']); ?>">

        <label>Notes / Comments</label>
        <textarea name="notes"><?php echo htmlspecialchars($student['notes']); ?></textarea>
